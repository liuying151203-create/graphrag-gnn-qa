...
- 默认使用后台处理，提交后显示任务 ID、当前阶段、百分比进度和手动刷新入口；取消勾选可切换为同步上传。
- 勾选覆盖选项后，重建相同内容对应的索引；内容变化会生成新的文档 ID。
- 对当前上传结果显式确认后，删除对应的 Milvus chunks、Neo4j 文档关系和可安全清理的孤立实体。

上传依赖 Embedding、LLM、Milvus 和 Neo4j 全部就绪，删除不依赖 LLM。后台任务状态保存在当前 FastAPI 进程内，后端重启后原任务 ID 会失效。页面顶部的数据规模来自本地预处理文件，不会随着 API 数据库操作即时变化；上传或删除结果以操作区返回的统计为准。
...
8. 展开文档管理区，用小型 TXT 提交后台入库任务，展示任务 ID、阶段进度和最终统计。
9. 展示覆盖重建和确认删除能力，最后说明快照降级、当前评估边界和进程内任务队列限制。
